...
class KNearestNeighbor(object):
  ...
  def compute_distances_one_loop(self, X):
    ...
    for i in range(num_test):
      ...
      #######################################################################
      xsqare = np.sum(np.square(X[i]))
      trainsqare = np.sum(np.square(self.X_train),axis=1)  # take a long time
      crossitem = np.dot(X[i],self.X_train.T)
      dists[i,:] =  np.sqrt( (xsqare + trainsqare) -2*crossitem)
      #######################################################################
      #                         END OF YOUR CODE                            #
      #######################################################################
    return dists
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    '''
    tmp1 = X.reshape((num_test,1,-1))
    tmp2 = self.X_train.reshape((1,num_train,-1))
    dists = np.linalg.norm( tmp2 - tmp1,axis=2)
    '''
